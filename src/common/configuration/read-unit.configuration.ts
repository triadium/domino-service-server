...
@ValidationSchema(Joi.object({
  ...
  UNIT_COUNT_FOR_QUEUE: Joi.number().integer().min(1).default(1),
  UNIT_QUEUE_CONCURRENCY: Joi.number().integer().min(1).default(100),
}))
export class ReadUnitConfiguration extends UnitConfiguration {
  constructor() {
    super('read-unit');
  }
}